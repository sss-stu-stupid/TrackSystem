...
bool ruleBasedFilter(vector<Point2f> pcPoints, float maxZ, int numPoints){
    ...
    if(numPoints < 2) return isPromising;  // 小于30个点，返回false 舍弃此聚类  cout << "numPoints is "<< numPoints <<endl;
...
void getBoundingBox(vector<PointCloud<PointXYZ>>  clusteredPoints,
    ...
    for (int iCluster = 0; iCluster < clusteredPoints.size(); iCluster++){  //  循环的次数就是 初始聚类ID数量numCluster 
        ...
        float initPX = floor((clusteredPoints[iCluster][0].x + roiM/2)*picScale); // (0 ~ 50) 第0个点的x，y坐标
        // float initPY = clusteredPoints[iCluster][0].y + roiM/2; // (0 ~ 50)
        float initPY = picScale*roiM - floor((clusteredPoints[iCluster][0].y + roiM/2)*picScale); // (0 ~ 50)
        // int initX = floor(initPX*picScale);  //   picScale = 900/roiM, 放大倍数900
        // int initY = floor(initPY*picScale); // 
        float offsetInitX = (roiM * picScale)/2 - initPX;  // 浮点偏移量
        float offsetInitY = (roiM * picScale)/2 - (picScale*roiM - initPY); 
        // int initPicX = initX;
        ...
        vector<Point2f> pointVec(numPoints);  // 定义
...
*/
          ...
          //  m.at<uchar>(offsetY, offsetX) = 255;
            pointVec[iPoint] = Point2f(offsetX, offsetY); // 偏移量
        ...
        if(slopeDist > lSlopeDist && (maxMy > 8 || maxMy < -5) && slopeDist < 15){ // int lSlopeDist = 1.0; 判断框的大小以及框内的点数; 过滤长条框
//        if(1){
            float maxDist = 0;
            float maxDx, maxDy;
...
//  候选框拟合处理
vector<PointCloud<PointXYZ>> boxFitting(PointCloud<PointXYZ>::Ptr point_cloud,
                array<array<int, numGrid>, numGrid> cartesianData,
                int numCluster,visualization_msgs::MarkerArray& ma)  // ma数据
{
    vector<PointCloud<PointXYZ>>  clusteredPoints(numCluster);
    getClusteredPoints(point_cloud, cartesianData, clusteredPoints);  // 指的是聚类点？具体什么意思呢？
